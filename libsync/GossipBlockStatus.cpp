...
void GossipBlockStatus::start()
{
    if (m_running)
    {
        GOSSIP_LOG(DEBUG) << LOG_DESC("GossipBlockStatus already started");
        return;
    }
    // start gossipBlockStatus worker
    startWorking();
    m_running = true;
    GOSSIP_LOG(DEBUG) << LOG_DESC("start GossipBlockStatus succ");
}
...
void GossipBlockStatus::stop()
{
    if (!m_running)
    {
        GOSSIP_LOG(DEBUG) << LOG_DESC("GossipBlockStatus already stopped");
        return;
    }
    GOSSIP_LOG(DEBUG) << LOG_DESC("stop GossipBlockStatus succ");
    m_running = false;
    doneWorking();
    if (isWorking())
    {
