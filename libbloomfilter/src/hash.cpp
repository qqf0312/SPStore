#include <libbloomfilter/bf/hash.hpp>
...
namespace bf {
default_hash_function::default_hash_function()  {
}
...
default_hasher::default_hasher(std::vector<default_hash_function> fns)
    : fns_(std::move(fns)) {
}

std::vector<digest> default_hasher::operator()(object const& o) const {
  // std::cout << "fns_.size() = " << fns_.size() << std::endl;
...
hasher make_hasher(size_t k, size_t seed, bool double_hashing) {
  ...
  // } else {
    std::vector<default_hash_function> fns(k);
