#include <libbloomfilter/bf/bloom_filter/basic.hpp>
#include <iostream>
#include <cassert>
#include <cmath>
...
basic_bloom_filter::basic_bloom_filter(double fp, size_t capacity, size_t seed,
  ...
  // std::cout << "required_cells = " << required_cells << std::endl;
...
basic_bloom_filter::basic_bloom_filter(hasher h, bitvector b)
    : hasher_(std::move(h)), bits_(std::move(b)), partition_(false) {
}
...
void basic_bloom_filter::add(object const& o) {
  ...
  // std::cout <<"after hasher" << std::endl;
  if (partition_) {
    ...
    // std::cout << "not partition" << std::endl;
    for (auto d : digests)
