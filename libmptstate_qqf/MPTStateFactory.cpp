    ...
    h256 const& _root, std::shared_ptr<dev::storage::TableFactory>)
{
    if (_root == dev::h256())
    {
        ...
        if(flag)return m_state;
        m_state = std::make_shared<MPTState>(m_accountStartNonce, m_db, BaseState::Empty);
        flag = true;
        return m_state;
    }
    else
    {
        ...
        m_state = std::make_shared<MPTState>(m_accountStartNonce, m_db, BaseState::PreExisting);
        // mptState->setRoot(_root);
        m_state->setRoot(_root);
        return m_state;
    }
}
std::shared_ptr<dev::mptstate::MPTState> MPTStateFactory::getMPTState()
{
    // auto mptState = std::make_shared<MPTState>(m_accountStartNonce, m_db, BaseState::Empty);
    return m_state;
}