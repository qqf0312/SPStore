...
// OverlayDB State::openDB(fs::path const& _basePath, h256 const&, WithExisting _we,State* s)
// {
...
{
    std::cout<<"State::addressHasCode"<<std::endl;
...
void State::createContract(Address const& _address)
{
    ...
    // m_contract_state.insert(pair<Address,SecureTrieDB<Address, OverlayDB>>(_address,
    // contract_state));
}
...
void State::setStorage(Address const& _contract, u256 const& _key, u256 const& _value)
{
    ...
    // Address data_contract;
    // bool iscompute = false;
    // string fout = "./nonce_t.txt";
    // std::ofstream out_key(fout, std::ofstream::app);
    // out_key  << _key << std::endl;
    // out_key.close();
...
{
    // std::cout << "AddressHash dev::mptstate::commit" << std::endl;
    AddressHash ret;
