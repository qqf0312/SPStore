...
ErasureCodeIsa::get_chunk_size(unsigned int object_size) const
{
  ...
  std::cout << "get_chunk_size: chunk_size " << chunk_size
           << " must be modulo " << alignment << std::endl;
  unsigned modulo = chunk_size % alignment;
  if (modulo) {
    std::cout << "get_chunk_size: " << chunk_size
             << " padded to " << chunk_size + alignment - modulo << std::endl;
                                  ...
                                  int blocksize)
{
  ...
  if (m == 1)
    ...
    ec_encode_data_base(blocksize, k, m, encode_tbls,
                   (unsigned char**) data, (unsigned char**) coding);
}
                                  ...
                                  int blocksize)
{
  ...
  if (m == 1) {
    ...
    // ceph_assert(1 == nerrs);
    std::cout << "isa_decode: reconstruct using region xor [" <<
      erasures[0] << "]" << std::endl;
  ...
  if (!tcache.getDecodingTableFromCache(erasure_signature, p_tbls, matrixtype, k, m)) {
    ...
    if (gf_invert_matrix(b, d, k) < 0) {
      std::cout << "isa_decode: bad matrix" << std::endl;
      return -1;
    }
...
ErasureCodeIsaDefault::prepare()
{
  ...
  if (!*p_enc_coeff) {
    std::cout << "[ cache tables ] creating coeff for k=" <<
      k << " m=" << m << std::endl;
  ...
  if (!*p_enc_table) {
    std::cout << "[ cache tables ] creating tables for k=" <<
      k << " m=" << m << std::endl;
  ...
  std::cout << "[ cache memory ] = " << memory_lru_cache << " bytes" <<
    " [ matrix ] = " <<
    ((matrixtype == kVandermonde) ? "Vandermonde" : "Cauchy") << std::endl;
