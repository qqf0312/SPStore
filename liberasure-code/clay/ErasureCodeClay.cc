                           ...
                           ostream *ss)
{
  ...
  pft.profile["m"] = '2';  
pft.profile["w"] = '8';
  mds.erasure_code = std::make_shared<ErasureCodeIsaDefault>(mds.tcache,ErasureCodeIsaDefault::kVandermonde);
  pft.erasure_code = std::make_shared<ErasureCodeIsaDefault>(pft.tcache,ErasureCodeIsaDefault::kVandermonde);
  mds.erasure_code->init(mds.profile,ss);
  pft.erasure_code->init(pft.profile,ss);
  t = (k + m + nu) / q;
  sub_chunk_no = pow_int(q, t);

  std::cout << __func__
<< " (k,m,w,nu,d,q,t,nu)=(" << k << ","<< m << ","<< w << ","<< nu << ","<< d << ","<< q << "," << t << "," << nu <<")" <<std::endl;
  return err;
}
                                       ...
                                       map<int, vector<pair<int, int>>> *minimum)
{
  ...
  {
    std::cout << "minimum_to_repair: shouldn't have come here" << endl;
    assert(0);
  }
                            ...
                            map<int, bufferlist> *repaired, int chunk_size)
{
  ...
  for (int i = 0; i < k + m; i++)
  {
    ...
    if (auto found = chunks.find(i); found != chunks.end())
    { // i is a helper
      if (i < k)
      {
