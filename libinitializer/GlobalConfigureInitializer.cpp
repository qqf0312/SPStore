...
void dev::initializer::initGlobalConfig(const boost::property_tree::ptree& _pt)
{
    ...
    g_BCOSConfig.binaryInfo.version = SCHAIN_PROJECT_VERSION;
    g_BCOSConfig.binaryInfo.buildTime = SCHAIN_BUILD_TIME;
    g_BCOSConfig.binaryInfo.buildInfo =
        string(SCHAIN_BUILD_PLATFORM) + "/" + string(SCHAIN_BUILD_TYPE);
    g_BCOSConfig.binaryInfo.gitBranch = SCHAIN_BUILD_BRANCH;
    g_BCOSConfig.binaryInfo.gitCommitHash = SCHAIN_COMMIT_HASH;
...
void dev::version()
{
    std::cout << "FISCO-BCOS Version : " << SCHAIN_PROJECT_VERSION << std::endl;
    std::cout << "Build Time         : " << SCHAIN_BUILD_TIME << std::endl;
    std::cout << "Build Type         : " << SCHAIN_BUILD_PLATFORM << "/"
              << SCHAIN_BUILD_TYPE << std::endl;
    std::cout << "Git Branch         : " << SCHAIN_BUILD_BRANCH << std::endl;
    std::cout << "Git Commit Hash    : " << SCHAIN_COMMIT_HASH << std::endl;
}
