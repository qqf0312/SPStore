...
if (NOT SCHAIN_BUILD_TYPE)
    set(SCHAIN_BUILD_TYPE "unknown")
endif()

if (NOT SCHAIN_BUILD_PLATFORM)
    set(SCHAIN_BUILD_PLATFORM "unknown")
endif()

execute_process(
    COMMAND git --git-dir=${ETH_SOURCE_DIR}/.git --work-tree=${ETH_SOURCE_DIR} rev-parse HEAD
    OUTPUT_VARIABLE SCHAIN_COMMIT_HASH OUTPUT_STRIP_TRAILING_WHITESPACE ERROR_QUIET
) 

if (NOT SCHAIN_COMMIT_HASH)
    set(SCHAIN_COMMIT_HASH 0)
endif()

execute_process(
    COMMAND git --git-dir=${ETH_SOURCE_DIR}/.git --work-tree=${ETH_SOURCE_DIR} diff HEAD --shortstat
    OUTPUT_VARIABLE SCHAIN_LOCAL_CHANGES OUTPUT_STRIP_TRAILING_WHITESPACE ERROR_QUIET
)

execute_process(
    COMMAND date "+%Y%m%d %H:%M:%S" 
    OUTPUT_VARIABLE SCHAIN_BUILD_TIME OUTPUT_STRIP_TRAILING_WHITESPACE ERROR_QUIET
    )

execute_process(
    COMMAND date "+%Y%m%d" 
    OUTPUT_VARIABLE SCHAIN_BUILD_NUMBER OUTPUT_STRIP_TRAILING_WHITESPACE ERROR_QUIET
    )

execute_process(
    COMMAND git rev-parse --abbrev-ref HEAD 
    OUTPUT_VARIABLE SCHAIN_BUILD_BRANCH OUTPUT_STRIP_TRAILING_WHITESPACE ERROR_QUIET
    )

if (SCHAIN_LOCAL_CHANGES)
    set(SCHAIN_CLEAN_REPO 0)
else()
    set(SCHAIN_CLEAN_REPO 1)
endif()
