...
namespace dev {
namespace mptstate {
OverlayDB MPTState::openDB(boost::filesystem::path const &_path,
...
void MPTState::transferBalance(Address const &_from, Address const &_to,
                               u256 const &_value) {
...
void MPTState::setStorage(Address const &_contract, u256 const &_location,
                          u256 const &_value) {
...
void MPTState::kill(Address _a) { m_state.kill(_a); }
...
void MPTState::incNonce(Address const &_id) { m_state.incNonce(_id); }
...
h256 MPTState::rootHash(bool) const { return m_state.rootHash(); }

void MPTState::commit() { m_state.commit(); }

void MPTState::dbCommit(h256 const &, int64_t) {
    // std::cout << "MPTState::dbCommit" << std::endl;
    m_state.db().commit();
}

void MPTState::setRoot(h256 const &_root) { m_state.setRoot(_root); }
...
size_t MPTState::savepoint() const { return m_state.savepoint(); }

void MPTState::rollback(size_t _savepoint) { m_state.rollback(_savepoint); }

void MPTState::clear() { m_state.cacheClear(); }

bool MPTState::checkAuthority(Address const &, Address const &) const {
    return true;
}

State &MPTState::getState() { return m_state; }
