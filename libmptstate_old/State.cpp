...
State::State(u256 const &_accountStartNonce, OverlayDB const &_db,
    ...
    if (_bs != BaseState::PreExisting)
        ...
        m_state.init();
    ifstream in("deployaddress.txt");
    string line;
    if (in) // 有该文件
    {
        while (getline(in, line)) // line中不包括每行的换行符
        {
            Address tmp = FixedHash<20>(line);
            m_compute_contract.insert(tmp);
        }
    } else // 没有该文件
    {
...
State::State(State const &_s)
    : m_db(_s.m_db), m_state(&m_db, _s.m_state.root(), Verification::Skip),
      m_cache(_s.m_cache), m_unchangedCacheEntries(_s.m_unchangedCacheEntries),
      m_nonExistingAccountsCache(_s.m_nonExistingAccountsCache),
      m_touched(_s.m_touched), m_accountStartNonce(_s.m_accountStartNonce),
...
OverlayDB State::openDB(fs::path const &_basePath, h256 const &,
                        WithExisting _we) {
    ...
    try {
        ...
        if (g_BCOSConfig.diskEncryption.enable)
            status = devdb::EncryptedLevelDB::Open(
        ...
        std::shared_ptr<db::DatabaseFace> dbFace =
            std::make_shared<devdb::LevelDB>(basicDB);

        LOG(INFO) << "statedb Opened state DB. PATH = " << path.string();
        return OverlayDB(dbFace);
    } catch (boost::exception const &ex) {
        LOG(WARNING) << boost::diagnostic_information(ex);
        if (fs::space(path).available < 1024) {
            LOG(WARNING) << "Not enough available space found on hard drive. "
                            "Please free some up "
                            "and then re-run. Bailing.";
            BOOST_THROW_EXCEPTION(NotEnoughAvailableSpace());
        } else {
            LOG(WARNING) << "Database " << (path)
                         << "already open. You appear to have another instance "
                            "of ethereum "
                            "running. Bailing.";
...
// OverlayDB State::openDB(fs::path const& _basePath, h256 const&, WithExisting
// _we,State* s)
// {
//     ifstream in("deployaddress.txt");
//     string line;
...
//         cout <<"no such file" << endl;
//     }
//     fs::path path = _basePath.empty() ? Defaults::get()->m_dbPath :
//     _basePath; path /= fs::path("state");

//     if (_we == WithExisting::Kill)
//     {
//         LOG(DEBUG) << "statedb: Killing state database
//         (WithExisting::Kill)."; fs::remove_all(path);
//     }
//     fs::create_directories(path);
//     DEV_IGNORE_EXCEPTIONS(fs::permissions(path, fs::owner_all));
...
//         if (g_BCOSConfig.diskEncryption.enable)
//             status =
//             devdb::EncryptedLevelDB::Open(devdb::LevelDB::defaultDBOptions(),
//                 path.string(), &basicDB,
//                 g_BCOSConfig.diskEncryption.cipherDataKey,
//                 g_BCOSConfig.diskEncryption.dataKey);
//         else
...
//         {
//             LOG(WARNING) << "Database " << (path)
//                          << "already open. You appear to have another
...
Account *State::account(Address const &_addr) {
    ...
    if (m_nonExistingAccountsCache.find(_addr) !=
        m_nonExistingAccountsCache.end())
        return nullptr;
    ...
    RLP state(stateBack);
    auto account = Account(state[0].toInt<u256>(), state[1].toInt<u256>(),
...
void State::clearCacheIfTooLarge() const {
    ...
    while (m_unchangedCacheEntries.size() > 1000) {
        ...
        swap(m_unchangedCacheEntries[randomIndex],
             m_unchangedCacheEntries.back());
...
unordered_map<Address, u256> State::addresses() const {
    ...
    BOOST_THROW_EXCEPTION(InterfaceNotSupported()
                          << errinfo_interface("State::addresses()"));
#endif
}

std::pair<State::AddressMap, h256> State::addresses(h256 const &_beginHash,
                                                    size_t _maxResults) const {
    AddressMap addresses;
    h256 nextKey;

#if ETH_FATDB
    for (auto it = m_state.hashedLowerBound(_beginHash);
         it != m_state.hashedEnd(); ++it) {
        auto const address = Address(it.key());
        auto const itCachedAddress = m_cache.find(address);

        // skip if deleted in cache
        if (itCachedAddress != m_cache.end() &&
            itCachedAddress->second.isDirty() &&
    ...
    // get addresses from cache with hash >= _beginHash (both new and old
    // touched, we can't distinguish them) and order by hash
    ...
    // if some new accounts were created in cache we need to return fewer
    // results
    if (addresses.size() > _maxResults) {
...
void State::createContract(Address const &_address) {
    ...
    // modified 2020/8
    // fs::path path = Defaults::get()->m_dbPath;
    // path /= fs::path("state");
    // path /= fs::path(_address.hex());
    // OverlayDB contract_db = openDB(path, sha3(""), BaseState::Empty);
    // SecureTrieDB<Address, OverlayDB> contract_state(&contract_db);
    // m_contract_state.insert(pair<Address,SecureTrieDB<Address,
    // OverlayDB>>(_address, contract_state));
}
...
void State::setStorage(Address const &_contract, u256 const &_key,
                       u256 const &_value) {
    // std::cout << "State::setStorage" << std::endl;
    // modified 2020/8
    Address data_contract;
    bool iscompute = false;
    string fout = "./key.txt";
    std::ofstream out_key(fout, std::ofstream::app);
    out_key << "W:" << _key << std::endl;
    out_key.close();
    // m_changeLog.emplace_back(_contract, _key, storage(_contract, _key));
    // m_cache[_contract].setStorage(_key, _value);
    // m_test_storage[_key] = _value;
}
...
#else
    (void)_id;
    BOOST_THROW_EXCEPTION(InterfaceNotSupported() << errinfo_interface(
...
template <class DB>
AddressHash dev::mptstate::commit(AccountMap const &_cache,
        ...
        if (i.second.isDirty()) {
            if (!i.second.isAlive())
                ...
                if (i.second.storageOverlay().empty()) {
                    ...
                    SecureTrieDB<h256, DB> storageDB(_state.db(),
                                                     i.second.baseRoot());
...
dev::mptstate::commit<OverlayDB>(AccountMap const &_cache,
                                 SecureTrieDB<Address, OverlayDB> &_state);
template AddressHash
dev::mptstate::commit<MemoryDB>(AccountMap const &_cache,
                                SecureTrieDB<Address, MemoryDB> &_state);
