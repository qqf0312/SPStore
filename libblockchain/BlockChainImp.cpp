    ...
    else
    {
        ...
        if (tb)
        {
            ...
            if (entries->size() > 0)
            {
                ...
                BLOCKCHAIN_LOG(INFO) << LOG_DESC("Get block from db")
...
std::shared_ptr<bytes> BlockChainImp::getBlockRLP(dev::h256 const& _blockHash, int64_t _blockNumber)
{
    ...
    if (bool(cachedBlock.first))
    {
        ...
        BLOCKCHAIN_LOG(INFO) << LOG_DESC("Get block RLP from cache")
                              << LOG_KV("getCacheTimeCost", getCache_time_cost)
                              << LOG_KV("totalTimeCost", utcTime() - start_time);
        return blockRLP;
    }
    else
    {
        ...
        if (tb)
        {
            ...
            if (entries->size() > 0)
            {
                ...
                BLOCKCHAIN_LOG(INFO) << LOG_DESC("Get block RLP from db")
                                      << LOG_KV("getCacheTimeCost", getCache_time_cost)
                                      << LOG_KV("openTableTimeCost", openTable_time_cost)
                                      << LOG_KV("selectTimeCost", select_time_cost)
...
std::shared_ptr<std::vector<dev::eth::NonceKeyType>> BlockChainImp::getNonces(int64_t _blockNumber)
{
    ...
    BLOCKCHAIN_LOG(INFO) << LOG_DESC("getNonces") << LOG_KV("blkNumber", _blockNumber);
...
std::shared_ptr<dev::executive::StateFactoryInterface> BlockChainImp::getstateFactory()
{
...
dev::bytes BlockChainImp::getHashNeed2Proof(uint32_t index, const dev::bytes& data)
{
    ...
    BLOCKCHAIN_LOG(INFO) << "transactionindex:" << index << " data:" << toHex(data)
...
void BlockChainImp::writeTxToBlock(const Block& block, std::shared_ptr<ExecutiveContext> context)
{
    ...
    if (tb && tb_nonces)
    {
        ...
        BLOCKCHAIN_LOG(INFO) << LOG_BADGE("WriteTxOnCommit")
...
// std::shared_ptr<dev::executive::StateFace> BlockChainImp::getMPTState(std::shared_ptr<dev::blockverifier::ExecutiveContext> context)
// {
//     return context->getMPTState();
// }
    ...
    std::shared_ptr<Block> block, std::shared_ptr<ExecutiveContext> context)
{
    auto start_time = utcTime();
    auto record_time = utcTime();
    // std::cout<<"CommitResult BlockChainImp::commitBlock"<<std::endl;
    ...
    try
    {
        ...
        record_time = utcTime();
        {
            ...
            BLOCKCHAIN_LOG(INFO) << LOG_BADGE("Commit")
        ...
        BLOCKCHAIN_LOG(INFO) << LOG_BADGE("Commit") << LOG_DESC("Commit block time record")
