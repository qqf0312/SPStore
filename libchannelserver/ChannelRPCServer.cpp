...
bool ChannelRPCServer::SendResponse(const std::string& _response, void* _addInfo)
{
    ...
    if (session)
    {
        ...
        message->setData((const uint8_t*)_response.data(), _response.size());
...
void dev::ChannelRPCServer::blockNotify(int16_t _groupID, int64_t _blockNumber)
{
    ...
    for (auto session : activedSessions)
    {
        ...
        {
            message->setTopicData(topic, (const uint8_t*)content.data(), content.size());
        }
        else
        {
            message->setTopicData(topic, (const uint8_t*)resp.data(), resp.size());
        }
                    ...
                    [serverRef, sessionRef, seq](const std::string& receiptContext) {
                        ...
                        if (server && session)
                        {
                            ...
                            channelMessage->setData(
                                (const uint8_t*)receiptContext.c_str(), receiptContext.size());
    ...
    try
    {
        ...
        auto respCallback = [serverRef, sessionRef](const std::string& _filterID, int32_t _result,
            ...
            if (server && session && session->actived())
            {
                ...
                channelMessage->setData((const uint8_t*)resp.c_str(), resp.size());
        ...
        message->setSeq(seq);
        message->setResult(0);
        message->setTopicData(std::string(""), (const uint8_t*)resp.data(), resp.size());
    ...
    dev::channel::ChannelSession::Ptr session, dev::channel::Message::Ptr message)
{
    try
    {
        ...
        Json::FastWriter writer;
        auto resp = writer.write(response);
        message->setData((const uint8_t*)resp.data(), resp.size());
        session->asyncSendMessage(message, dev::channel::ChannelSession::CallbackType(), 0);
    ...
    dev::channel::ChannelSession::Ptr session, dev::channel::Message::Ptr message)
{
    ...
    if (session->protocolVersion() == ProtocolVersion::v1)
    {
        ...
        {
            data = "1";
            message->setData((const uint8_t*)data.data(), data.size());
            session->asyncSendMessage(message, dev::channel::ChannelSession::CallbackType(), 0);
        }
    }
    // v2 and v3 for now
