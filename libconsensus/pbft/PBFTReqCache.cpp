...
void PBFTReqCache::delCache(dev::eth::BlockHeader const& _highestBlockHeader)
{
    ...
    PBFTReqCache_LOG(INFO) << LOG_DESC("delCache") << LOG_KV("hash", hash.abridged());
...
bool PBFTReqCache::checkViewChangeReq(ViewChangeReq::Ptr _req, int64_t const& _blockNumber)
{
    ...
    {
        PBFTReqCache_LOG(INFO) << LOG_DESC("return without addViewChangeReq for lower blockHeight")
    ...
    {
        PBFTReqCache_LOG(INFO) << LOG_DESC("return without addViewChangeReq for lower view")
...
void PBFTReqCache::addViewChangeReq(ViewChangeReq::Ptr _req, int64_t const& _blockNumber)
{
    ...
    PBFTReqCache_LOG(INFO) << LOG_DESC("addViewChangeReq") << LOG_KV("reqIdx", _req->idx)
