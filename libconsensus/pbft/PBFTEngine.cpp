...
bool PBFTEngine::shouldSeal()
{
    ...
    // std::cout << "PBFTEngine::shouldSeal;time = " << std::string(buffer) <<  "      " <<m_cfgErr 
    // << "     " <<  (m_accountType != NodeAccountType::SealerAccount) << std::endl;
...
//std::cout << "check getLeader;time = " << std::string(buffer) <<  "    " <<ret.first << "     " <<  (ret.second != nodeIdx())  << "    " << (m_notifyNextLeaderSeal && getNextLeader() == nodeIdx()) << "    " << (m_reqCache->committedPrepareCache().height == m_consensusBlockNumber) << "  " << (m_reqCache->rawPrepareCacheHeight() != m_consensusBlockNumber) << std::endl;
    if (!ret.first)
    {
        return false;
    }
    ...
    {
        // std::cout << "m_reqCache->committedPrepareCache().height = " << m_reqCache->committedPrepareCache().height << std::endl;
        // std::cout << "m_reqCache->rawPrepareCacheHeight() = " << m_reqCache->rawPrepareCacheHeight()<< std::endl;
...
void PBFTEngine::reloadMsg(std::string const& key, PBFTMsg* msg)
{
    ...
    try
    {
        ...
        if (data.empty())
        {
            PBFTENGINE_LOG(INFO) << LOG_DESC("reloadMsg: Empty message stored")
                                  << LOG_KV("nodeIdx", nodeIdx())
                                  << LOG_KV("nodeId", m_keyPair.pub().abridged());
            return;
        }
        msg->decode(ref(data), 0);
        PBFTENGINE_LOG(INFO) << LOG_DESC("reloadMsg") << LOG_KV("fromIdx", msg->idx)
                              << LOG_KV("nodeId", m_keyPair.pub().abridged())
                              << LOG_KV("H", msg->height)
    ...
    {
        PBFTENGINE_LOG(INFO) << LOG_DESC("reloadMsg from db failed")
...
PrepareReq::Ptr PBFTEngine::constructPrepareReq(dev::eth::Block::Ptr _block)
{
    ...
    auto currentTimes = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
    strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", localtime(&currentTimes));
    // std::cout << "start constructPrepareReq ;time = " << std::string(buffer) <<"leader = "<<getLeader().second<< "block number = " <<_block->header().number()<< std::endl;
    ...
    if (m_enablePrepareWithTxsHash && prepareReq->pBlock->transactions()->size() > 0)
    {
        ...
        m_threadPool->enqueue([this, prepareReq, sendedData]() {
            try
            {
                ...
                auto currentTimes = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
                strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", localtime(&currentTimes));
                // std::cout << "sendPrepareMsgFromLeader ;time = " << std::string(buffer) << "leader = "<<getLeader().second<< "block number = " <<prepareReq->pBlock->header().number()<<std::endl;
...
bool PBFTEngine::generatePrepare(dev::eth::Block::Ptr _block)
{
    ...
    auto currentTimes = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
    strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", localtime(&currentTimes));
    // std::cout << "after constructPrepareReq ;time = " << std::string(buffer) << "leader = "<<getLeader().second<< "block number = " <<_block->header().number() <<std::endl;
    //std::cout << "generatePrepare block size = " << prepareReq->block->size()<<std::endl;
...
if (prepareReq->pBlock->getTransactionSize() == 0 && m_omitEmptyBlock)
    ...
    currentTimes = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
    strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", localtime(&currentTimes));
    // std::cout << "before handlePrepareMsg ;time = " << std::string(buffer) << "leader = "<<getLeader().second<< "block number = " <<_block->header().number()<<std::endl;
    handlePrepareMsg(prepareReq);
...
bool PBFTEngine::getNodeIDByIndex(h512& nodeID, const IDXTYPE& idx) const
{
    ...
    if (nodeID == h512())
    {
        PBFTENGINE_LOG(INFO) << LOG_DESC("getNodeIDByIndex: not sealer") << LOG_KV("Idx", idx)
                              << LOG_KV("myNode", m_keyPair.pub().abridged());
        return false;
    }
    return true;
...
bool PBFTEngine::broadcastViewChangeReq()
{
    ViewChangeReq req(
        m_keyPair, m_highestBlock.number(), m_toView, nodeIdx(), m_highestBlock.hash());
    PBFTENGINE_LOG(INFO) << LOG_DESC("broadcastViewChangeReq ") << LOG_KV("v", m_view)
    ...
    if (!m_fastViewChange)
    {
        PBFTENGINE_LOG(INFO) << LOG_DESC("ViewChangeINFO: not caused by omit empty block ")
                                << LOG_KV("v", m_view) << LOG_KV("toV", m_toView)
                                << LOG_KV("curNum", m_highestBlock.number())
                                << LOG_KV("hash", req.block_hash.abridged())
                                << LOG_KV("nodeIdx", nodeIdx())
    ...
    std::shared_ptr<dev::h512s> forwardNodes)
{
    ...
    for (auto session : sessions)
    {
        if (session.nodeID() == nodeId)
        {
            ...
            PBFTENGINE_LOG(INFO) << LOG_DESC("sendMsg") << LOG_KV("packetType", packetType)
...
void PBFTEngine::execBlock(Sealing& sealing, PrepareReq const& req, std::ostringstream&)
{
   ...
   // }
    auto decode_time_cost = utcTime() - record_time;
     record_time = utcTime();
    //std::cout << " sealing.block size = " <<  sealing.block->blockSize() <<std::endl;
    ...
    // checkBlockValid(*(sealing.block));
     ...
     record_time = utcTime();
    PBFTENGINE_LOG(INFO)<< LOG_KV("checkBlockValid time = ", utcTime() - start_time);
    ...
    // m_txPool->verifyAndSetSenderForBlock(*sealing.block);
     ...
     record_time = utcTime();
    PBFTENGINE_LOG(INFO)<< LOG_KV("verifyAndSetSenderForBlock time = ", utcTime() - start_time);
    ...
    P2PMessage::Ptr message, std::function<void(PBFTMsgPacket::Ptr)> const& _f)
{
    ...
    {
        PBFTENGINE_LOG(INFO) << LOG_DESC("onRecvPBFTMessage: illegal msg ")
    ...
    PrepareReq::Ptr _prepareReq, std::ostringstream& _oss)
{
    ...
    Sealing workingSealing(m_blockFactory);
        //std::cout << "execPrepareAndGenerateSignMsg block size = " << _prepareReq->block->size()<<std::endl;
...
try
    ...
    {
        PBFTENGINE_LOG(INFO) << LOG_DESC("Block execute failed") << LOG_KV("INFO", _oss.str())
                                << LOG_KV("EINFO", boost::diagnostic_information(e));
        return true;
    }
    ...
    auto currentTimes = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
    strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", localtime(&currentTimes));
    // std::cout << "before broadcastSignReq ;time = " << std::string(buffer) << "leader = "<<getLeader().second<<"block number = " <<_prepareReq->pBlock->header().number()<<std::endl;
...
void PBFTEngine::checkAndCommit()
{
    ...
    {
        PBFTENGINE_LOG(INFO) << LOG_DESC("checkAndCommit, SignReq enough")
        ...
        {
            PBFTENGINE_LOG(INFO) << LOG_DESC("checkAndCommit: InvalidView")
        ...
        PBFTENGINE_LOG(INFO) << LOG_DESC("checkAndCommit: broadcastCommitReq")
        ...
        auto currentTimes = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
        strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", localtime(&currentTimes));
        // std::cout << "before broadcastCommitReq ;time = " << std::string(buffer) << "leader = "<<getLeader().second<<"block number = " <<m_reqCache->prepareCache().pBlock->header().number()<<std::endl;
        if (!broadcastCommitReq(m_reqCache->prepareCache()))
        {
            PBFTENGINE_LOG(INFO) << LOG_DESC("checkAndCommit: broadcastCommitReq failed");
        }

        m_timeManager.m_lastSignTime = utcSteadyTime();
        PBFTENGINE_LOG(INFO) << LOG_DESC("checkAndCommit: broadcastCommitReq before checkandsave")
                              << LOG_KV("time = ", utcTime() - start_time);
        checkAndSave();
    }
}
...
void PBFTEngine::checkAndSave()
{
    ...
    if (sign_size >= minValidNodeSize && commit_size >= minValidNodeSize)
    {
        ...
        if (m_reqCache->prepareCache().view != m_view)
        {
            PBFTENGINE_LOG(INFO) << LOG_DESC("checkAndSave: InvalidView")
                                  << LOG_KV("prepView", m_reqCache->prepareCache().view)
                                  << LOG_KV("view", m_view)
...
// std::cout << "CommitBlock Succ " << "prepareHeight = " << m_reqCache->prepareCache().height 
                // << ";reqIdx = "  << m_reqCache->prepareCache().idx << ";hash = " << m_reqCache->prepareCache().block_hash.abridged() 
                // << ";nodeIdx = " << nodeIdx() << std::endl;                
m_reqCache->delCache(m_reqCache->prepareCache().pBlock->blockHeader());
    ...
    PBFTENGINE_LOG(INFO)
                << LOG_DESC("checkAndSave finished ")
                << LOG_KV("time = ", utcTime() - start_commit_time);
}
    ...
    ViewChangeReq::Ptr viewChange_req, PBFTMsgPacket const& pbftMsg)
{
    ...
    PBFTENGINE_LOG(INFO) << LOG_DESC("handleViewChangeMsg Succ ") << oss.str();
    return true;
}
...
void PBFTEngine::collectGarbage()
{
    ...
    {
        PBFTENGINE_LOG(INFO) << LOG_DESC("collectGarbage")
                              << LOG_KV(
...
void PBFTEngine::handleMsg(PBFTMsgPacket::Ptr pbftMsg)
{
    ...
    default:
    {
        PBFTENGINE_LOG(INFO) << LOG_DESC("handleMsg:  Err pbft message")
...
// get the forwardNodes
// _printLog is true when viewChangeINFO to show more detailed info
    ...
    std::shared_ptr<P2PSession> _session, P2PMessage::Ptr _message)
{
    try
    {
        PBFTENGINE_LOG(INFO) << LOG_DESC("onReceiveGetMissedTxsRequest")
    ...
    {
        PBFTENGINE_LOG(INFO) << LOG_DESC("onReceiveGetMissedTxsRequest exceptioned")
                                << LOG_KV("peer", _session->nodeID().abridged())
    ...
    NetworkException _exception, std::shared_ptr<P2PSession> _session, P2PMessage::Ptr _message)
{
    ...
    {
        PBFTENGINE_LOG(INFO) << LOG_DESC("handleP2PMessage: invalid message")
                                << LOG_KV("peer", _session->nodeID().abridged())
                                << LOG_KV("errorInfo", boost::diagnostic_information(_e));
    }
}
    ...
    std::shared_ptr<P2PSession> _session, P2PMessage::Ptr _message)
{
    ...
    {
        PBFTENGINE_LOG(INFO) << LOG_DESC("onReceiveMissedTxsResponse exceptioned")
                                << LOG_KV("peer", _session->nodeID().abridged())
                                << LOG_KV("errorInfo", boost::diagnostic_information(_e));
    }
}
