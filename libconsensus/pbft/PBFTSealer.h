...
protected:
    ...
    bool shouldHandleBlock() override
    {
        ...
        // std::cout << "start SealBlock ;time = " << std::string(buffer) << 
        //  "  m_sealing.block->blockHeader().number() = " <<m_sealing.block->blockHeader().number()<<
        //  "  m_blockChain->number() + 1 = " << (m_blockChain->number() + 1) << "  m_pbftEngine->getLeader().first = "
        //  <<m_pbftEngine->getLeader().first <<"  m_pbftEngine->getLeader().second = " <<m_pbftEngine->getLeader().second 
        // <<"  m_pbftEngine->nodeIdx()="<< m_pbftEngine->nodeIdx() <<std::endl;
...
private:
    ...
    void resetBlockForViewChange()
    {
        ...
        /// reset
        {
            ...
            PBFTSEALER_LOG(INFO) << LOG_DESC("resetSealingBlock for viewchange")
                                  << LOG_KV(
    ...
    void resetBlockForNextLeader(dev::h256Hash const& filter)
    {
        {
            WriteGuard l(x_sealing);
            PBFTSEALER_LOG(INFO) << LOG_DESC("resetSealingBlock for nextLeader")
                                  << LOG_KV(
                                         "sealingNumber", m_sealing.block->blockHeader().number())
                                  << LOG_KV("curNum", m_blockChain->number());
            //modified 2020/7/26
