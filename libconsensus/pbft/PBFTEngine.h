...
protected:
    ...
    inline CheckResult checkReq(T& req, std::ostringstream& oss) const
    {
        ...
        {
            PBFTENGINE_LOG(INFO) << LOG_DESC("checkReq: Is Syncing higher number")
        ...
        std::shared_ptr<dev::p2p::P2PSession> _session)
    {
        ...
        if (valid)
        {
            ...
            {
                PBFTENGINE_LOG(INFO) << "[decodeToRequests] Invalid network-received packet";
                return false;
            }
            _req->setOtherField(
