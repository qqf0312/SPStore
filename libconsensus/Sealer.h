...
public:
        ...
        m_consensusEngine(nullptr)
    {
        ...
        // m_signalled.wait_for(l, boost::chrono::milliseconds(5000));
...
protected:
    ...
    virtual bool checkTxsEnough(uint64_t maxTxsCanSeal)
    {
        ...
        if (enough)
        {
            SEAL_LOG(INFO) << "[checkTxsEnough] Tx enough: [txNum]: " << tx_num;
        }
        return enough;
    }
    ...
    {
        SEAL_LOG(INFO) << "[resetSealingBlock]" << LOG_KV("blkNum", m_blockChain->number())
...
protected:
    ...
    uint64_t m_maxBlockCanSeal = 2000;
