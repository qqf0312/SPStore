...
void RaftEngine::initRaftEnv()
{
...

    {
        ...
        std::cout << "m_minElectTimeoutInit =" << m_minElectTimeoutInit << "  m_maxElectTimeoutInit =  " << m_maxElectTimeoutInit
                  << "m_minElectTimeoutBoundary =" << m_minElectTimeoutBoundary << "  m_maxElectTimeoutBoundary =  " << m_maxElectTimeoutBoundary
                  << "m_heartbeatTimeout =" << m_heartbeatTimeout << "  m_heartbeatInterval =  " << m_heartbeatInterval
                  << "m_increaseTime =" << m_increaseTime << std::endl;
    }
...
bool RaftEngine::runAsLeaderImp(std::unordered_map<h512, unsigned> &memberHeartbeatLog)
{
    ...
    {
        RAFTENGINE_LOG(INFO) << LOG_DESC("[#runAsLeaderImp]Heartbeat Timeout");
...
void RaftEngine::runAsLeader()
{
    ...
    while (isWorking())
    {
      ...
      //  auto end_time = utcTime() - start_time;
       // RAFTENGINE_LOG(INFO) << "end sync time =  "
         //                    << end_time;
...
bool RaftEngine::runAsCandidateImp(VoteState &_voteState)
{
    ...
    {
        RAFTENGINE_LOG(INFO) << LOG_DESC("[#runAsCandidateImp]VoteState")
...
void RaftEngine::runAsCandidate()
{
    ...
    {
        RAFTENGINE_LOG(INFO) << LOG_DESC("[#runAsCandidate]Won election, switch to leader now");
        switchToLeader();
        return;
    }
...
bool RaftEngine::runAsFollowerImp()
{
    ...
    if (checkElectTimeout())
    {
        RAFTENGINE_LOG(INFO) << LOG_DESC("[#runAsFollowerImp]Elect timeout, switch to Candidate");
        switchToCandidate();
        return false;
    }
...
void RaftEngine::broadcastHeartbeat()
{
    ...
    if (interval >= m_heartbeatInterval)
    {
        ...
        clearFirstVoteCache();
        RAFTENGINE_LOG(INFO) << LOG_DESC("[#broadcastHeartbeat]Heartbeat broadcasted");
    }
    else
    {
...
void RaftEngine::broadcastVoteReq()
{
    ...
    if (voteReqMsg)
    {
        ...
        RAFTENGINE_LOG(INFO) << LOG_DESC("[#broadcastVoteReq]VoteReq broadcasted");
    }
    else
    {
        RAFTENGINE_LOG(INFO) << LOG_DESC("[#broadcastVoteReq]Failed to broadcast VoteReq");
    }
}
...
{
    RAFTENGINE_LOG(INFO) << LOG_DESC("[#handleVoteRequest]") << LOG_KV("from", _from)
    ...
    RAFTENGINE_LOG(INFO) << LOG_DESC("[#handleVoteRequest]Grant vreq");
...
{
    RAFTENGINE_LOG(INFO) << LOG_DESC("[#handleHeartbeat]") << LOG_KV("fromIdx", _from)
    ...
    {
        RAFTENGINE_LOG(INFO) << LOG_DESC("[#handleHeartbeat]Discard hb for smaller term")
...
void RaftEngine::recoverElectTime()
{
    ...
    RAFTENGINE_LOG(INFO) << LOG_DESC("[#recoverElectTime]Reset elect time to init")
...
{
    RAFTENGINE_LOG(INFO) << LOG_DESC("[#sendResponse]Ready to send response") << LOG_KV("to", _to)
    ...
    u256 const &_from, h512 const &_node, RaftVoteResp const &_resp, VoteState &_vote_state)
{
    ...
    {
        RAFTENGINE_LOG(INFO) << LOG_DESC("[#handleVoteResponse]Peer's term is smaller than mine")
    ...
    default:
    {
        RAFTENGINE_LOG(INFO) << LOG_DESC("[#handleVoteResponse]Error voteFlag")
...
void RaftEngine::increaseElectTime()
{
    ...
    RAFTENGINE_LOG(INFO) << LOG_DESC("[#increaseElectTime]Increase elect time")
                         << LOG_KV("minElectTimeout", m_minElectTimeout)
                         << LOG_KV("maxElectTimeout", m_maxElectTimeout);
}
...
{
    {
        ...
        for (auto iter = m_memberBlock.begin(); iter != m_memberBlock.end(); ++iter)
        {
            ...
            RAFTENGINE_LOG(INFO) << LOG_DESC("[#shouldSeal]Wait to download block")
                                 << LOG_KV("iter->second.height", iter->second.height) << LOG_KV("currentHeight", currentHeight)
                                 << LOG_KV("iter->second.block_hash", iter->second.block_hash) << LOG_KV("currentBlockHash", currentBlockHash);
...

    {
        ...
        if (bool(m_uncommittedBlock))
        {
            RAFTENGINE_LOG(INFO) << LOG_DESC("[#shouldSeal]Wait to commit uncommitted block")
...
bool RaftEngine::commit(Block const &_block)
{
    ...
    m_commitReady = false;
    RAFTENGINE_LOG(INFO) << LOG_DESC("[#commit]Wait to commit block")
    ...
    {
        RAFTENGINE_LOG(INFO) << LOG_DESC("[#commit]I'm not the leader anymore, stop committing");
        return false;
    }

    RAFTENGINE_LOG(INFO) << LOG_DESC("[#commit]Start to commit block");
...
void RaftEngine::execBlock(Sealing &_sealing, Block const &_block)
{
    ...
    RAFTENGINE_LOG(INFO) << LOG_DESC("[#execBlock]")
...
void RaftEngine::checkAndSave(Sealing &_sealing)
{
    ...
    if (ret == CommitResult::OK)
    {
        ...
        m_uncommittedBlockNumber = 0;
        ul.unlock();
        RAFTENGINE_LOG(INFO) << LOG_DESC("[#checkAndSave]Commit block succ");
