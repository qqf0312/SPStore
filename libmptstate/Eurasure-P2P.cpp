...
void EurasureP2P::processPacket(SyncMsgPacket::Ptr packet, NodeAddr const& destnodeId)
{
    ...
    case ECRequestChunkPacket: {
        ...
        while (m_eurasure->getCompleteEpoch() < coding_epoch){
            sleep(0.1);
            std::cout<< "m_eurasure->getCompleteEpoch() : " << m_eurasure->getCompleteEpoch()
                << "  coding_epoch" << coding_epoch << std::endl;
        }
    ...
    case ECReponseChunkPacket: {
        ...
        // std::cout<<"Merkle list = "<<merkle_list<<std::endl;
        ...
        std::cout << "ECReponseChunkPacket get str = " << str << std::endl;
        ...
        // std::cout << "GetChunkDataKey(coding_epoch, group_id, chunk_pos), merkle_list[0])"
        //     <<m_eurasure->GetChunkDataKey(coding_epoch, group_id, chunk_pos)<<std::endl;
        // if(chunk_data_map.find(m_eurasure->GetChunkDataKey(coding_epoch, group_id, chunk_pos)) == chunk_data_map.end()){
        //     std::cout<<"cant find in map"<<std::endl;
        // }
        // }
