    ...
    const std::map<std::string, std::string>& _kvMap, const std::string& key)
{
    ...
    // 这时候更新了m_commit(mainCommit),其实也可以吃从db里读取对应的承诺，具体可见方法coverMain
    ...
    std::cout<< "vo = " << vo.substr(0, len - 3 * pacl) << std::endl;
        ...
        [&](const tbb::blocked_range<int>& _r) {
            ...
            for (int pos = _r.begin(); pos != _r.end(); ++pos)
            {
                ...
                for (auto it = accounts.begin(); it != accounts.end();)
                {
                    ...
                    std::string newValue(*it);
                    std::cout << " VCGroup::updateCommit value = " << newValue<< std::endl;
                    it++;
...
bool VCGroup::recoverMain()
{
    ...
    for (int pos = 0; pos < mainVCTemplate->m_size; pos++)
    {
        ...
        // std::cout << "subcommit " << DecToHex(blockNumber) + "_subcommit" + DecToHex(pos)
        //               << " recoverMain "
        //               << std::endl;
        // std::cout<<"read value from vcstorage = "<< value <<std::endl;
        if (status.ok())
