...
{
    // std::cout << "ExecutiveContextFactory::initExecutiveContext" << std::endl;
    ...
    if(!flag)
    {
        statefaces = m_stateFactoryInterface->getState(stateRoot, memoryTableFactory);
        context->setState(statefaces);
        flag = true;
    }
    else
    {
