...
void Service::heartBeat()
{
    ...
    {
        //std::cout << "staticnode id = " << it.second << std::endl;
        /// exclude myself
        if (it.second == id())
        {
            SERVICE_LOG(INFO) << LOG_DESC("heartBeat ignore myself nodeID same")
        ...
        {
            SERVICE_LOG(INFO) << LOG_DESC("heartBeat ignore connected")
        ...
        {
            SERVICE_LOG(INFO) << LOG_DESC("heartBeat ignore invalid address");
            continue;
        }
        SERVICE_LOG(INFO) << LOG_DESC("heartBeat try to reconnect")
                           << LOG_KV("endpoint", it.first.name());
        m_host->asyncConnect(
...
void Service::setWhitelist(PeerWhitelist::Ptr _whitelist)
{
    ...
    checkWhitelistAndClearSession();
    SERVICE_LOG(INFO) << LOG_BADGE("Whitelist") << LOG_DESC("Set whitelist")
...
void Service::checkWhitelistAndClearSession()
{
    if (m_whitelist != nullptr && m_whitelist->enable())
    {
        ...
        for (auto session : sessions)
        {
            ...
            SERVICE_LOG(INFO) << LOG_BADGE("Whitelist")
    ...
    std::shared_ptr<dev::network::SocketFace> const& _s, dev::network::NodeID const& nodeID)
{
    ...
    {
        SERVICE_LOG(INFO) << LOG_DESC("updateStaticNodes") << LOG_KV("nodeID", nodeID.abridged())
                           << LOG_KV("endpoint", endpoint.name());
        it->second = nodeID;
    }
    else
    {
        SERVICE_LOG(INFO) << LOG_DESC("updateStaticNodes can't find endpoint")
                           << LOG_KV("nodeID", nodeID.abridged())
                           << LOG_KV("endpoint", endpoint.name());
    }
}
...
P2PMessage::Ptr Service::sendMessageByNodeID(NodeID nodeID, P2PMessage::Ptr message)
{
    try
    {
        ...
        SERVICE_LOG(INFO) << LOG_DESC("sendMessageByNodeID mutex unlock");
    ...
    CallbackFuncWithSession callback, dev::network::Options options)
{
    try
    {
        ...
        if (it != m_sessions.end() && it->second->actived())
        {
            ...
            {
                //  std::cout << "send no callback" << std::endl;
...
{
    NodeIDs nodeIDsToSend = getPeersByTopic(topic);
    SERVICE_LOG(INFO) << LOG_DESC("asyncMulticastMessageByTopic")
...
void Service::asyncBroadcastMessage(P2PMessage::Ptr message, dev::network::Options options)
{
    try
    {
        ...
        for (auto s : sessions)
        {
            std::cout<<"nodeid"<<s.first<<std::endl;
...
void Service::registerHandlerByProtoclID(PROTOCOL_ID protocolID, CallbackFuncWithSession handler)
{
    ...
    // std::cout << "finish register handle protocol id = " << protocolID << std::endl;
}
...
NodeIDs Service::getPeersByTopic(std::string const& topic)
{
    ...
    SERVICE_LOG(INFO) << LOG_DESC("getPeersByTopic") << LOG_KV("topic", topic)
