...
public:
    ...
    virtual void onConnect(dev::network::NetworkException e, const dev::network::NodeInfo& nodeInfo,
    ...
    h512s getNodeListByShardID(GROUP_ID groupID) override { return m_shardID2NodeList[groupID]; }
    void setShardID2NodeList(std::map<GROUP_ID, h512s> _groupID2NodeList) override
    {
        RecursiveGuard l(x_shardList);
        m_shardID2NodeList = _groupID2NodeList;
    }

    void setNodeListByShardID(GROUP_ID _groupID, const h512s& _nodeList) override
    {
        RecursiveGuard l(x_shardList);
        m_shardID2NodeList[_groupID] = _nodeList;
    }
...
private:
    ...
    mutable RecursiveMutex x_shardList;
    std::map<GROUP_ID, h512s> m_shardID2NodeList;   
