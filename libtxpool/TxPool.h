...
public:
    ...
    TxPool(std::shared_ptr<dev::p2p::P2PInterface> _p2pService,
        ...
        PROTOCOL_ID const& _protocolId, uint64_t const& _limit = 1020400, uint64_t workThreads = 32)
    ...
    std::shared_ptr<dev::eth::Transactions> topTransactions(
        uint64_t const& _limit, h256Hash& _avoid, bool _updateAvoid = true) override;
