...
{
    CACHED_STORAGE_LOG(DEBUG) << "Init flushStorage thread";
    m_taskThreadPool =
...
{
    CACHED_STORAGE_LOG(DEBUG) << "CachedStorage commit: " << datas.size() << " num: " << num;
    ...
    if (m_backend)
    {
        ...
        if (!disabled())
        {
            ...
            STORAGE_LOG(DEBUG) << "Submited block task: " << num
    ...
    {
        STORAGE_LOG(DEBUG) << "No backend storage, skip commit...";

        setSyncNum(num);
    }
     //CACHED_STORAGE_LOG(DEBUG) << "CachedStorage commit time = : " << (utcTime() - start_time) << num;
    return total;
}
...
void CachedStorage::stop()
{
    if (!m_running)
    {
        STORAGE_LOG(DEBUG) << LOG_DESC("CachedStorage already stopped!");
        return;
    }
    STORAGE_LOG(DEBUG) << "Stopping flushStorage thread";
    m_running->store(false);
    m_taskThreadPool->stop();
    ...
    STORAGE_LOG(DEBUG) << "flushStorage thread stopped.";
}
...
bool CachedStorage::commitBackend(Task::Ptr task)
{
    ...
    STORAGE_LOG(DEBUG) << "Start commit block: " << task->num << " to backend storage";
    try
    {
...
void CachedStorage::checkAndClear()
{
   ...
   // auto startTime = utcSteadyTime();
    ...
    //std::cout << "check and clear time = " << utcSteadyTime()-startTime <<std::endl;
    if (clearThrough > 0)
    {
        CACHED_STORAGE_LOG(DEBUG) << "Clear finished, total: " << clearCount << " entries, "
